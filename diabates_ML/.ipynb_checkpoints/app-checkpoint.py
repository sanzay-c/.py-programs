...
def create_feature_importance_chart():
    ...
    # Sample feature importance (you can load actual values from model)
...
def main():
    ...
    if page == "🏠 Home & Prediction":
        ...
        **Neural Network Architecture:**
        - Input Layer: Patient health features
        ...
        with col1:
            ...
            st.write("- Age group categorization")
            st.write("- BMI categorization")
            st.write("- Glucose level categorization")
            st.write("- Feature interactions (BMI × Age)")
            st.write("- One-hot encoding for categories")
            
        with col2:
            st.write("**Data Cleaning:**")
            st.write("- Invalid zero values handled")
            st.write("- Missing value imputation")
            st.write("- Standard scaling for numerical features")
            st.write("- Train/Validation/Test: 70/15/15 split")
